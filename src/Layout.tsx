...
import { IconBell, IconCompass, IconHome, IconLoop, IconPlus, IconUser, IconWallet } from './icons'
    ...
    <div className="app-shell">
        ...
        <nav>
          {items.map((it) => (
            <NavLink key={it.to} to={it.to} end={it.end} className={({ isActive }) => `nav-link ${isActive ? 'active' : ''}`}>
              ...
              {it.label === 'Activity' && unread > 0 && <span className="chip gold" style={{ marginLeft: 'auto' }}>{unread}</span>}
            </NavLink>
          ))}
          <NavLink to="/wallet" className={({ isActive }) => `nav-link ${isActive ? 'active' : ''}`}><IconWallet size={18} />Wallet</NavLink>
        </nav>
        {me && (
          <div className="sidebar-foot">
            <NavLink to="/app/profile" className="nav-link">
              <Avatar user={me} size="sm" />
              <span>{me.name}<div className="tiny">{me.balance} Favors</div></span>
            </NavLink>
          </div>
        )}
      </aside>
      ...
      <nav className="bottom-nav">
        {items.map((it) => {
          ...
          if (it.to === '/app/post') return <NavLink key={it.to} to={it.to} className={active ? 'active' : ''}><span className="post-fab"><IconPlus /></span></NavLink>
          return <NavLink key={it.to} to={it.to} end={it.end} className={active ? 'active' : ''}><it.icon size={20} />{it.label}</NavLink>
        })}
      </nav>
    </div>
  )
